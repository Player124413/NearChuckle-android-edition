...
void CXGame::SaveConfiguration( const char *pszSystemCfg,const char *pszGameCfg,const char *sProfileName)
{
	...
	FILE *pFile = NULL;
#ifdef __ANDROID__
	// Android: write the VF_DUMPTODISK cvar dump (same content as the PC
	// system.cfg) into mobile.cfg. Never into system.cfg itself - that name is
	// reserved for desktop configs copied into the game folder and is cleaned up.
	if (!sSystemCfg.empty())
	{
		pFile=fxopen(sSystemCfg.c_str(), "wb");
		if (pFile)
		{
			fputs("-- [Mobile-Configuration]\r\n", pFile);
...
{			
#ifdef __ANDROID__
	// Android: restore the saved mobile configuration (cvar dump written by
	// SaveConfiguration) instead of system.cfg. This point runs after the
	// renderer/3DEngine registered all their cvars, so graphics quality
	// settings (r_Quality_*, e_*, ...) are applied correctly. CSystemConfiguration
	...
	m_pSystem->LoadConfiguration("mobile.cfg");
#else
